...
//update
const updateEm = async (PERSONAL_ID, EMPLOYMENT_CODE, CURRENT_MIDDLE_NAME, CURRENT_LAST_NAME, CURRENT_FIRST_NAME, SOCIAL_SECURITY_NUMBER,) => {
    const updatedLastName = CURRENT_MIDDLE_NAME + ' ' + CURRENT_LAST_NAME;

    let [results, fields] = await connection.query(
        'UPDATE `mydb`.`employee` SET `EmployeeNumber`=?, `LastName` = ?, `FirstName` = ?, `SSN` = ? WHERE `idEmployee` = ?;',
        [EMPLOYMENT_CODE, updatedLastName, CURRENT_FIRST_NAME, SOCIAL_SECURITY_NUMBER, PERSONAL_ID]
    )
}
